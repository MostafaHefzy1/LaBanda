...
class MealsProduct extends StatelessWidget {
  CategoryModel model;

  MealsProduct(this.model);

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<HomeCubit, HomeStates>(
      listener: (context, state) {
        if (state is! GetMealsProductSuccessSate)
                  ...
                  Container(
                    child: GridView.count(
                      ...
                      children: List.generate(
                        HomeCubit.get(context).mealsProductModel.length,
                                ...
                                MaterialPageRoute(
                                    builder: (_) => DetailsProduct(
                                          HomeCubit.get(context)
                                              .mealsProductModel[index],
                                        )));
                          },
                          child: defaultProductItem(
                              HomeCubit.get(context).mealsProductModel[index]),
                        ),
                      ),
                    ),
                  ),
