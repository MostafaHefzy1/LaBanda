...
class PastaProduct extends StatelessWidget {
  CategoryModel model;

  PastaProduct(this.model);

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<HomeCubit, HomeStates>(
      listener: (context, state) {
        if (state is! GetPastaProductSuccessSate)
                  ...
                  Container(
                    child: GridView.count(
                      ...
                      children: List.generate(
                        HomeCubit.get(context).pastaProductModel.length,
                                ...
                                MaterialPageRoute(
                                    builder: (_) => DetailsProduct(
                                          HomeCubit.get(context)
                                              .pastaProductModel[index],
                                        )));
                          },
                          child: defaultProductItem(
                              HomeCubit.get(context).pastaProductModel[index]),
                        ),
                      ),
                    ),
                  ),
