...
class SandwichesProduct extends StatelessWidget {
  CategoryModel model;

  SandwichesProduct(this.model);

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<HomeCubit, HomeStates>(
      listener: (context, state) {
        if (state is! GetSandwichesProductSuccessSate)
                  ...
                  Container(
                    child: GridView.count(
                      ...
                      children: List.generate(
                        HomeCubit.get(context).sandwichesProductModel.length,
                                ...
                                MaterialPageRoute(
                                    builder: (_) => DetailsProduct(
                                          HomeCubit.get(context)
                                              .sandwichesProductModel[index],
                                        )));
                          },
                          child: defaultProductItem(HomeCubit.get(context)
                              .sandwichesProductModel[index]),
                        ),
                      ),
                    ),
