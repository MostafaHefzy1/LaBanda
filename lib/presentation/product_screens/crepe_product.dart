...
class CrepeProduct extends StatelessWidget {
  CategoryModel model;

  CrepeProduct(this.model);

  @override
  Widget build(BuildContext context) {
    return BlocConsumer<HomeCubit, HomeStates>(
      listener: (context, state) {
        if (state is! GetCrepeProductSuccessSate)
                  ...
                  Container(
                    child: GridView.count(
                      ...
                      children: List.generate(
                        HomeCubit.get(context).crepeProductModel.length,
                                ...
                                MaterialPageRoute(
                                    builder: (_) => DetailsProduct(
                                          HomeCubit.get(context)
                                              .crepeProductModel[index],
                                        )));
                          },
                          child: defaultProductItem(
                              HomeCubit.get(context).crepeProductModel[index]),
                        ),
                      ),
                    ),
                  ),
